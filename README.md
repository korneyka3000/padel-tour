...
**Готовы движок, хранилище и Telegram-бот.** Турнир можно провести из чата: собрать состав
галочками, вводить счёт в два тапа и видеть таблицу — всё в одном сообщении, которое бот
перерисовывает вместо того, чтобы засыпать чат. Впереди веб со статистикой.
...
uv run padel-tour-bot          # Telegram-бот (нужен BOT_TOKEN в .env)
...
группах (`cli`, `db`), чтобы `import padel_tour.engine` ничего лишнего не тянул.
