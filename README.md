...
**Сейчас готова первая веха — движок турниров.** Это чистая Python-библиотека без БД, HTTP
и UI: она знает правила и больше ничего. Всё остальное — API, Telegram-бот, веб — будет
насаживаться на неё.
...
uv run padel-tour play         # провести турнир в терминале
uv run padel-tour demo         # прогнать оба формата со случайными результатами
...
Движок не имеет зависимостей вообще. CLI живёт в опциональной группе `cli` (typer), чтобы
`import padel_tour.engine` ничего лишнего не тянул.
...
|---|---|
| M1 ✅ | Движок турниров |
| M2 | Персистентность и FastAPI |
| M3 | Telegram-бот на aiogram 3 |
| M4 | Веб на React со статистикой и графиками |
